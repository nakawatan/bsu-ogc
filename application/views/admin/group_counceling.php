    ...
    <div class="btn-list-style-2 btn-months btn-two-col <?= $get_month ? 'hide' : '' ?>">
        <?php 
            foreach (allow_month() as $month) {
                echo '<a href="'.LINK.'admin/group_counseling/'.$month.'" class="accordion">'.$month.'</a>';
            }
        ?>
    </div>
    ...
    <div class="btn-list-style-2 btn-days <?= !empty($get_month) ? '' : 'hide' ?>">
        <?php 
            foreach (allow_days_of_week() as $day) :
                echo '<a href="'.LINK.'admin/group_counseling/'.$get_month.'/'.$day.'">'.$day.'</a>';
        ?>
            ...
            <table>
                ...
                <?php foreach($get_appointments as $appointments) : ?>
                    <tr>
                        ...
                        <td><a href="<?= LINK ?>admin/update_appointment_gc/<?= $appointments['date'] ?>" class="btn btn-green btn-small">View Appointments</a></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
