<form class="form-validate" id="update_cgmc_tf" action="./update_cgmc_tf/<?= $data_table['id'] ?>" method="post" enctype="multipart/form-data">
  ...
  <!-- $transferee_cgmc_form_file & $transferee_cgmc_form_status is not yet on database.
		<div class="form-group mb-5">
          ...
          <label class="btn-attach" for="transferee_cgmc_form_file" data-target="<?= base_url('assets/uploads/docs/').encodeFolder($data_table['student_id']).'/'.$data_table['$transferee_cgmc_form_file'] ?>"><?= $data_table['$transferee_cgmc_form_file']; ?></label>   
          <div class="approved txt-green <?= $data_table['transferee_cgmc_form_status'] == 'pending' || $data_table['transferee_cgmc_form_status'] == 'reject' ? 'hide' : '' ?>" data-target="transferee_cgmc_form_status"><i class="fas fa-check"></i></div>
          <div class="approved txt-red <?= $data_table['transferee_cgmc_form_status'] == 'pending' || $data_table['transferee_cgmc_form_status'] == 'approved' ? 'hide' : '' ?>" data-target="transferee_cgmc_form_status"><i class="fas fa-times"></i></div>
        </div>
    <div class="form-group text-right mt-0">
        <div><input type="hidden" name="transferee_cgmc_form_status" value="<?= $data_table['transferee_cgmc_form_status'] ?>"></div>
        <div class="d-right">
